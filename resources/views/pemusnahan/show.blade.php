...
<div class="content-header-right col-md-6 col-12">
    <div class="btn-group float-md-right">
        @if ($pemusnahan->status != 'Proses pengajuan')
            <a href="/pemusnahan/index" class="btn btn-info">Kembali</a>        
        @else
            @if (count($pemusnahan->detail_pemusnahan) > 0)
                <form action="/pemusnahan/update/{{ $pemusnahan->id }}?status=selesai" method="post">
                    @csrf
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                ...
                <h4 class="card-title">Form Pengajuan Pemusnahan</h4>
            </div>
                ...
                <div class="card-body">
                    <div class="form-body">
                        <div class="form-group">
                            ...
                            <input type="text"  class="form-control" placeholder="Nomor Pengajuan" name="no_pengajuan" value="{{ $pemusnahan->no_pengajuan }}" readonly>
                        </div>
                    </div>
                </div>
            </div>
...
<div class="row">
    ...
    <div class="col-md-4">
        <div class="card">
            ...
            <div class="card-content collpase show">
                <div class="card-body">
                    <form action="/detailPemusnahan/store" method="post">
                        @csrf
                        <input type="hidden" name="id_pemusnahan" value="{{ $pemusnahan->id }}">
                        <div class="form-group">
                            <label>Barang</label>
    ...
    @endif
    <div class="{{ $pemusnahan->status == 'Proses pengajuan' ? 'col-md-8' : 'col-md-12' }}">
        <div class="card">
            <div class="card-header">
                <a class="heading-elements-toggle"><i class="la la-ellipsis-v font-medium-3"></i></a>
                <div class="heading-elements">
            ...
            <div class="card-content collpase show">
                <div class="card-body">
                    <table class="table table-responsive datatable d-lg-table">
                        ...
                        <tbody>
                            @foreach ($pemusnahan->detail_pemusnahan as $data)
                                <tr>
                                    <td>{{ $data->asset->nama }}, Kode: {{ $data->asset->kode }}, Kondisi: {{ $data->asset->kondisi }}</td>
                                    @if ($data->asset->status_pemusnahan == "True")
                                    <td>Dimusnahkan</td>
                                    @else
                                    <td>Belum dimusnahkan</td>
                                    @endif
                                    @if ($pemusnahan->status == 'Proses pengajuan')
                                    <td>
...
<div class="modal fade" id="delete">
    ...
    <div class="modal fade" id="confirm">
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                <form id="confirmForm" action="/pemusnahan/update/{{ $pemusnahan->id }}?status=dikonfirmasi" method="POST">
    
                    @csrf
                    <!-- Modal Header -->
                    <div class="modal-header bg-info">
                        <h4 class="modal-title text-white">Apakah anda yakin ingin mengkonfirmasi Pemusnahan ini ?</h4>
                        <button type="button" class="close text-white" data-dismiss="modal">&times;</button>
                    </div>
    ...
    <div class="modal fade" id="reject">
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                <form id="rejectForm" action="/pemusnahan/update/{{ $pemusnahan->id }}?status=ditolak" method="POST">
    
                    @csrf
                    <!-- Modal Header -->
                    <div class="modal-header bg-info">
                        <h4 class="modal-title text-white">Apakah anda yakin ingin menolak Pemusnahan ini ?</h4>
                        <button type="button" class="close text-white" data-dismiss="modal">&times;</button>
                    </div>
...
@section('script')
    ...
    $(document).on("click", ".tombolHapus", function(){
        var id = $(this).val();
        $("#deleteForm").attr("action", "/detailPemusnahan/destroy/"+ id);
        $("#deleteId").val(id);
        $("#delete").modal();
    });
